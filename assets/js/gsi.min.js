$(document).ready(function() {
   ...
   const render = function() {       
    if((typeof window.localStorage.getItem('msg') !== 'undefined') && (window.localStorage.getItem('msg') !== null)) {
        ...
        if(typeof data.stats.next24h.prodWh !== 'undefined') {
                ...
                for(let j=1;j<12;j++) {
                    ...
                    for(let x=0;x<24;x++) {
                        ...
                        if(sum > maxGsi ) {
                            maxGsi = sum;
                            timeStamp = data.stats.next24h.prodWh[x].time;
                        }
                    }            
                    ...
                    for(let i=0;((i<24)&&(i<data.stats.next24h.prodWh.length));i++) {
                        if((data.stats.next24h.prodWh[i].time>= timeStamp) && (data.stats.next24h.prodWh[i].time<= timeStamp+ (3600000 * j))) {
                            row+="<td style='background:rgb(12,109,52);' title='"+new Date(data.stats.next24h.prodWh[i].time).toLocaleTimeString()+"'>&nbsp;</td>";
                        } else {
                            row+="<td style='background:rgb(253,168,37);' title='"+new Date(data.stats.next24h.prodWh[i].time).toLocaleTimeString()+"'>&nbsp;</td>";
                        }
                    }            
                    row+="</tr>";            
                    matrixhtml += '' + row;
