...
function changeCID(cid) {
    ...
    if(typeof window.mcids[cid] !== 'undefined') {
        window.localStorage.setItem('msg',window.mcids[cid].content);
    }
...
$(document).ready(function() {
    const render = async function(data) {
        window.mcids = data;                
        let peernames = {};
        if((typeof window.localStorage.getItem("peernames") !== 'undefined')&&(window.localStorage.getItem("peernames") !== null)) {
            peernames = JSON.parse(window.localStorage.getItem("peernames"));
        }
              
        let html ='<a class="dropdown-item" href="javascript:resetCID();">Mein Casa</a>';
        $('#lblPeername').html('');   
        let html_p2pstatus = '';
        let csum=0;
       for (const [key, value] of Object.entries(data)) {
           ...
           if(typeof peernames[key] !== 'undefined') {
               peername = peernames[key];
           } else {
               
                peername = content.name;
                peernames[key] = peername;
           }
           let active = '';  
           if(window.localStorage.getItem('cid')==key) {
               active=' active';
               $('#lblPeername').html(peername);
           }
           if(value.on < new Date().getTime() - 2*3600000) {
                active+=' text-muted';     
           }
           html+='<a class="dropdown-item'+active+'" id="peer_'+key+'" title="'+key+'" href="javascript:changeCID(\'' +key+'\')">'+peername+'</a>';      
           if(typeof content.latest.power_w !== 'undefined') {
               //content.latest.power_w *=-1;
                let cc = '#ffa700';
                if(content.latest.power_w<250) cc ='#FFDD98';
                if(content.latest.power_w<50) cc ='#86df9b';
                if(content.latest.power_w<-250) cc ='#006c32';
                html_p2pstatus+='<div class="row"><div class="col-1" style="color:'+cc+'" title="'+content.latest.power_w+' W">⬤</div><div class="col-auto">'+peername+'</div></div>';
               csum+=content.latest.power_w;
            }
        }
        $('#p2pselection').show();
        $('#cids').html(html);
        ...
        if($('#p2ptab').length >0) {
            $('#p2ptab').html(html_p2pstatus);   
            $('#hcsum').attr('title',Math.round(csum/1000)+" kW");
            if(csum>0) {
                 $('#hcsum').css('color','#ffa700');
            } else {
                $('#hcsum').css('color','#006c32');
            }
    ...
    const refresh = function() {
        try {
            ...
            $.getScript( "assets/js/ipfs.min.js",async function() {
                console.log('Switches to P2P via in Browser IPFS');
