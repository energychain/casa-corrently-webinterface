...
$(document).ready(function() {
   ...
   const render = function() {
          ...
          if(lastPrice !== data.stats[selector].energyPrice_kwh) {
             ...
             if(typeof data.stats[selector].saldo_wh !== 'undefined') {
                 ...
                 for(let i=0;i<data.stats[selector].saldo_wh.saldo.length;i++) {
                        ...
                        if((j >23)&&((selector=='last7d')||(selector=='last30d')||(selector=='last90d')||(selector=='last180d'))) {                            
    ...
    $('#modErz').on('input change',function() {
        $('#erz').html(($('#modErz').val()).toLocaleString('de-DE',{'minimumFractionDigits':3,'maximumFractionDigits':3}));
    });
    ...
    $('#modBedarf').on('input change',function() {
        $('#bedarf').html(($('#modBedarf').val()).toLocaleString('de-DE',{'minimumFractionDigits':3,'maximumFractionDigits':3}));
    });
      } catch(e) {
          console.log(e);
          window.localStorage.removeItem('msg');
   ...
   update();
    $('#cbStrompreis').click(function() {
        let historyurl = "./history";
        if((typeof window.localStorage.getItem('cid') !== 'undefined') &&(window.localStorage.getItem('cid')!==null))  {
            historyurl = './ipfs?cid='+window.localStorage.getItem('history');            
        }
        $('#modalStrompreis').modal();
         $.getJSON(historyurl,function(data2)                   
         {                       
             let data = {};
             data.localHistory = data2;
             let dlast24h = [];
             let dlast7d = [];
             let dlast30d = [];
             let dlast90d = [];
             ...
             if((typeof data.localHistory !== 'undefined') && ( typeof data.localHistory.sort !== 'undefined')) {
                data.localHistory.sort(function(a,b) {
                 if (a.time > b.time) return 1;
                  if (b.time > a.time) return -1;
                  return 0;
                });
			for(let i=0;i<data.localHistory.length;i++) {
                if((historyurl == './history')||(data.localHistory[i].uuid==window.localStorage.getItem('cid'))) {
                 if(typeof data.localHistory[i].stats.last24h !== 'undefined') {
                    dlast24h.push({
                        y:data.localHistory[i].stats.last24h * 1,
                        x:data.localHistory[i].time * 1
                    });
                  }
                 if(typeof data.localHistory[i].stats.last7d !== 'undefined') {
                    dlast7d.push({
                        y:data.localHistory[i].stats.last7d * 1,
                        x:data.localHistory[i].time * 1
                    });
                  }                 
                 if(typeof data.localHistory[i].stats.last30d !== 'undefined') {
                    dlast30d.push({
                        y:data.localHistory[i].stats.last30d * 1,
                        x:data.localHistory[i].time * 1
                    });
                  }
                 if(typeof data.localHistory[i].stats.last90d !== 'undefined') {
                    dlast90d.push({
                        y:data.localHistory[i].stats.last90d * 1,
                        x:data.localHistory[i].time * 1
                    });
                  }
                 if(typeof data.localHistory[i].stats.last180d !== 'undefined') {
                    dlast180d.push({
                        y:data.localHistory[i].stats.last180d * 1,
                        x:data.localHistory[i].time * 1
                    });
                  }        
                 if(typeof data.localHistory[i].stats.last365d !== 'undefined') {
                    dlast365d.push({
                        y:data.localHistory[i].stats.last365d * 1,
                        x:data.localHistory[i].time * 1
                    });
                  }                   
                }
              }
                 let ctxAB = $('#trendChart');
                 let myChartAB = new Chart(ctxAB, {
                  type: 'line',
                  data: {
                    datasets: [{
    ...
    $('#collapplaner').on('hide.bs.collapse',function() {
       location.reload(); 
    });
});
